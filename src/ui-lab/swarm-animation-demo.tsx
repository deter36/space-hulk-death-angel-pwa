...
export default function SwarmAnimationDemo({ assetBase }: { assetBase: string }) {
  ...
  return <main className="lab-game-view" style={style}>
    ...
    <FormationBoard rows={rows} marineSpriteUrl={`${assetBase}/marine-idle.gif`} alienSpriteUrl={`${assetBase}/alien-attack.gif`} broodlordSpriteUrl="../../game-art/broodlord/idle.png" alienAttackStripUrl="../../game-art/genestealer/attack.png" alienDeathStripUrl="../../game-art/genestealer/death.png" swarmAnimationStates={animation ? { [TARGET_KEY]: animation } : {}} swarmStates={{ [TARGET_KEY]: "targeted" }} onInspect={setInspection} />
    <div className="lab-animation-dock"><div><span>Target swarm · right side · row 1</span><strong>{count} Genestealer{count === 1 ? "" : "s"} remaining</strong></div><button type="button" onClick={playAttack} disabled={Boolean(animation) || count === 0}>Attack</button><button type="button" onClick={playDeath} disabled={Boolean(animation) || count === 0}>Kill lead</button><button type="button" onClick={reset}>Reset</button></div>
