...
function Flank({ alienAttackStripUrl, alienDeathStripUrl, alienIdleStripUrl, alienSpriteUrl, broodlordSpriteUrl, flank, moving, onInspect, onSelectSwarm, onSelectTerrain, overlay, side, swarmAnimation, swarmState = "neutral", terrainState = "neutral" }: { alienAttackStripUrl?: string; alienDeathStripUrl?: string; alienIdleStripUrl?: string; alienSpriteUrl: string; broodlordSpriteUrl?: string; flank: LabFlank; moving?: boolean; onInspect?: (details: LabInspection) => void; onSelectSwarm: () => void; onSelectTerrain: () => void; overlay?: LabOverlayChoice; side: Side; swarmAnimation?: "attack" | "death"; swarmState?: LabTargetState; terrainState?: LabTargetState }) {
  ...
  return (
    <div className={`lab-flank lab-flank-${side.toLowerCase()} ${flank.swarm ? "is-engaged" : ""} ${flank.terrain ? "has-terrain" : ""} ${moving ? "is-swarm-moving" : ""}`}>
...
function Marine({ animation, deathStripUrl, dodgeStripUrl, marine, marineSpriteUrl, moveChoice, onInspect, onSelect, state }: { animation?: "dead" | "death" | "dodge"; deathStripUrl?: string; dodgeStripUrl?: string; marine: LabMarine; marineSpriteUrl: string; moveChoice?: LabMarineMoveChoice; onInspect?: (details: LabInspection) => void; onSelect: () => void; state: LabTargetState }) {
  const { facing, name, supportTokens = 0, team } = marine;
  const details = MARINE_DETAILS[name] ?? { range: 0 };
  const animationUrl = animation === "death" || animation === "dead" ? deathStripUrl : animation === "dodge" ? dodgeStripUrl : undefined;
