...
type FormationBoardProps = {
  ...
  onSelectTerrain?: (row: number, side: Side, terrainId?: string) => void;
  ...
  swarmCounterArtworkUrl?: string;
  terrainStates?: Record<string, LabTargetState>;
  terrainSpriteUrls?: Partial<Record<string, string>>;
};
...
function inspectTerrain(terrain: LabTerrain, controlPanelActionText?: string): LabInspection {
  const body = terrain.name === "Control Panel" && controlPanelActionText
    ? `Activate: ${controlPanelActionText}`
    : TERRAIN_RULES[terrain.name] ?? "No additional Terrain text is loaded for this preview.";
  return { eyebrow: `${terrain.color} spawn Terrain`, title: terrain.name, body };
}

function SwarmReadout({ swarm, artworkUrl }: { swarm: LabSwarm; artworkUrl?: string }) {
  const count = swarm.icons.length + (swarm.broodLords ?? 0);
  return <div className={`lab-swarm-readout ${artworkUrl ? "has-production-counter" : ""}`} style={artworkUrl ? { "--swarm-counter-art": `url(${artworkUrl})` } as CSSProperties : undefined} aria-label={`${count} Genestealers`}><b>{count}</b></div>;
}

function TerrainPiece({ controlPanelActionText, index, onDismissHoverInspection, onHoverInspect, onInspect, onSelect, state = "neutral", terrain, terrainSpriteUrls }: { controlPanelActionText?: string; index: number; onDismissHoverInspection?: () => void; onHoverInspect?: (details: LabInspection, anchor: DOMRect) => void; onInspect?: (details: LabInspection) => void; onSelect: () => void; state?: LabTargetState; terrain: LabTerrain; terrainSpriteUrls?: Partial<Record<string, string>> }) {
  const inspection = inspectTerrain(terrain, controlPanelActionText);
  const press = usePress(onSelect, () => onInspect?.(inspection), onHoverInspect ? (anchor) => onHoverInspect(inspection, anchor) : undefined, onDismissHoverInspection);
  return <button type="button" className={`lab-terrain ${terrainSpriteUrls?.[terrain.name] ? "has-art" : ""} lab-spawn-${terrain.color.toLowerCase()} is-${state}`} style={{ "--terrain-layer": index } as CSSProperties} aria-label={`${terrain.name} Terrain`} aria-disabled={state === "unavailable"} {...press}>{terrainSpriteUrls?.[terrain.name] && <img src={terrainSpriteUrls[terrain.name]} alt="" />}{terrain.supportTokens ? <b className="lab-terrain-support">{"●".repeat(terrain.supportTokens)}</b> : null}</button>;
}

function Flank({ alienAttackStripUrl, alienDeathStripUrl, alienIdleStripUrl, alienSpriteUrl, broodlordAttackStripUrl, broodlordDeathStripUrl, broodlordSpriteUrl, controlPanelActionText, flank, moving, movementDirection, onInspect, onHoverInspect, onDismissHoverInspection, onSelectSwarm, onSelectTerrain, overlay, side, swarmAnimation, swarmCounterArtworkUrl, swarmState = "neutral", terrainStates = {}, terrainSpriteUrls }: { alienAttackStripUrl?: string; alienDeathStripUrl?: string; alienIdleStripUrl?: string; alienSpriteUrl: string; broodlordAttackStripUrl?: string; broodlordDeathStripUrl?: string; broodlordSpriteUrl?: string; controlPanelActionText?: string; flank: LabFlank; moving?: boolean; movementDirection?: "up" | "down" | "flank"; onInspect?: (details: LabInspection) => void; onHoverInspect?: (details: LabInspection, anchor: DOMRect) => void; onDismissHoverInspection?: () => void; onSelectSwarm: () => void; onSelectTerrain: (terrainId?: string) => void; overlay?: LabOverlayChoice; side: Side; swarmAnimation?: "attack" | "death"; swarmCounterArtworkUrl?: string; swarmState?: LabTargetState; terrainStates?: Record<string, LabTargetState>; terrainSpriteUrls?: Partial<Record<string, string>> }) {
  ...
  const swarmPress = usePress(onSelectSwarm, () => flank.swarm && onInspect?.(inspectSwarm(flank.swarm)), flank.swarm && onHoverInspect ? (anchor) => onHoverInspect(inspectSwarm(flank.swarm!), anchor) : undefined, onDismissHoverInspection);
  return (
    <div className={`lab-flank lab-flank-${side.toLowerCase()} ${flank.swarm ? "is-engaged" : ""} ${flank.terrain ? "has-terrain" : ""} ${moving ? `is-swarm-moving is-swarm-moving-${movementDirection ?? "up"}` : ""}`}>
      {terrains.map((terrain, index) => <TerrainPiece key={terrain.id ?? `${terrain.name}.${index}`} terrain={terrain} index={index} state={terrainStates[terrain.id ?? ""] ?? "neutral"} terrainSpriteUrls={terrainSpriteUrls} controlPanelActionText={controlPanelActionText} onSelect={() => onSelectTerrain(terrain.id)} onInspect={onInspect} onHoverInspect={onHoverInspect} onDismissHoverInspection={onDismissHoverInspection} />)}
      {flank.swarm && <div className={`lab-swarm-target is-${swarmState}`} style={{ inset: 0, width: "100%", height: "100%", left: "auto", right: "auto", pointerEvents: "none" }}><button type="button" className="lab-swarm-hitbox" style={side === "LEFT" ? { position: "absolute", zIndex: 9, top: "12%", bottom: "10%", width: "58%", right: "4%", left: "auto", border: 0, padding: 0, background: "transparent", pointerEvents: "auto" } : { position: "absolute", zIndex: 9, top: "12%", bottom: "10%", width: "58%", left: "4%", right: "auto", border: 0, padding: 0, background: "transparent", pointerEvents: "auto" }} aria-label={`${flank.swarm.icons.length + (flank.swarm.broodLords ?? 0)} Genestealer swarm`} aria-disabled={swarmState === "unavailable"} {...swarmPress} /><span className="lab-swarm-sprites">{visibleMembers.map((member, index) => {
      ...
      })}</span><SwarmReadout swarm={flank.swarm} artworkUrl={swarmCounterArtworkUrl} /></div>}
...
export default function FormationBoard({ alienAttackStripUrl, alienDeathStripUrl, alienIdleStripUrl, alienSpriteUrl, broodlordAttackStripUrl, broodlordDeathStripUrl, broodlordSpriteUrl, collapsingMarine, controlPanelActionText, locationProgress, marineAnimationStates = {}, marineDeathStripUrl, marineDodgeStripUrl, marineFireStripUrls, marineJamStripUrls, marineSpriteUrl, marineMoveChoices = [], moveSlots = [], movingSwarmCell, movingSwarmCells = {}, marineStates = {}, onInspect, onHoverInspect, onDismissHoverInspection, onMarineMoveChoice, onMoveSlot, onOverlayChoice, onSelectMarine, onSelectSwarm, onSelectTerrain, overlayChoices = [], rows, selectedMarine, swarmAnimationStates = {}, swarmCounterArtworkUrl = "/space-hulk-death-angel-pwa/game-art/ui/counters/green-counter-proof.png", swarmStates = {}, terrainStates = {}, terrainSpriteUrls = {} }: FormationBoardProps) {
    ...
    <section className="lab-board" aria-label="Formation geometry preview">{locationProgress && <div className="lab-location-route" aria-hidden="true">{["left", "right"].map((side) => <div key={side} className={`lab-location-route-${side}`}>{route.map((tier) => { const active = tier === locationProgress; return <span key={tier} className={active ? "is-current" : ""}><img src={`/space-hulk-death-angel-pwa/game-art/location-stencils/${tier}-${active ? "active" : "inactive"}.png`} alt="" /></span>; })}</div>)}</div>}<div className="lab-formation">
      {rows.map((row, index) => {
        ...
        return <Fragment key={`${row.marine.team}.${row.marine.name}`}>{renderMoveSlot(index)}<div className={`lab-combat-row ${collapsingMarine === row.marine.name ? "is-collapsing" : ""}`}><span className="lab-row-number">{String(index + 1).padStart(2, "0")}</span>
          <Flank alienSpriteUrl={alienSpriteUrl} alienAttackStripUrl={alienAttackStripUrl} alienDeathStripUrl={alienDeathStripUrl} alienIdleStripUrl={alienIdleStripUrl} broodlordSpriteUrl={broodlordSpriteUrl} broodlordAttackStripUrl={broodlordAttackStripUrl} broodlordDeathStripUrl={broodlordDeathStripUrl} controlPanelActionText={controlPanelActionText} terrainSpriteUrls={terrainSpriteUrls} swarmCounterArtworkUrl={swarmCounterArtworkUrl} flank={row.left} moving={movingSwarmCell === cellKey(index, "LEFT") || Boolean(movingSwarmCells[cellKey(index, "LEFT")])} movementDirection={movingSwarmCells[cellKey(index, "LEFT")]} side="LEFT" swarmAnimation={swarmAnimationStates[cellKey(index, "LEFT")]} swarmState={swarmStates[cellKey(index, "LEFT")]} terrainStates={terrainStates} overlay={leftOverlay} onInspect={onInspect} onHoverInspect={onHoverInspect} onDismissHoverInspection={onDismissHoverInspection} onSelectSwarm={() => leftOverlay ? onOverlayChoice?.(leftOverlay) : onSelectSwarm?.(index, "LEFT")} onSelectTerrain={(terrainId) => onSelectTerrain?.(index, "LEFT", terrainId)} />
          ...
          <Flank alienSpriteUrl={alienSpriteUrl} alienAttackStripUrl={alienAttackStripUrl} alienDeathStripUrl={alienDeathStripUrl} alienIdleStripUrl={alienIdleStripUrl} broodlordSpriteUrl={broodlordSpriteUrl} broodlordAttackStripUrl={broodlordAttackStripUrl} broodlordDeathStripUrl={broodlordDeathStripUrl} controlPanelActionText={controlPanelActionText} terrainSpriteUrls={terrainSpriteUrls} swarmCounterArtworkUrl={swarmCounterArtworkUrl} flank={row.right} moving={movingSwarmCell === cellKey(index, "RIGHT") || Boolean(movingSwarmCells[cellKey(index, "RIGHT")])} movementDirection={movingSwarmCells[cellKey(index, "RIGHT")]} side="RIGHT" swarmAnimation={swarmAnimationStates[cellKey(index, "RIGHT")]} swarmState={swarmStates[cellKey(index, "RIGHT")]} terrainStates={terrainStates} overlay={rightOverlay} onInspect={onInspect} onHoverInspect={onHoverInspect} onDismissHoverInspection={onDismissHoverInspection} onSelectSwarm={() => rightOverlay ? onOverlayChoice?.(rightOverlay) : onSelectSwarm?.(index, "RIGHT")} onSelectTerrain={(terrainId) => onSelectTerrain?.(index, "RIGHT", terrainId)} />
