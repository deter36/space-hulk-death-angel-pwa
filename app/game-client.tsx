    ...
    <main className={`mission-shell ${choosingActions ? "is-choosing-actions" : ""}`} style={{ "--desktop-ui-scale": desktopUiScale.toFixed(3) } as CSSProperties}>
      <section className={`lab-hud ${missionInfoCollapsed ? "is-collapsed" : "is-expanded"}`} aria-label="Mission status">
        <div className="lab-hud-command">
          ...
          <div className="live-hud-tools"><button type="button" className="live-hud-undo" aria-label="Undo last action" disabled={!undoStatus.allowed} onClick={onUndo}>↶</button><div className="live-hud-menu"><button type="button" aria-label="Open game menu" aria-expanded={menuOpen} onClick={() => { setMenuOpen((current) => !current); setMenuScreen("ROOT"); }}>☰</button>{menuOpen && <div className="live-hud-menu-panel">{menuScreen === "ROOT" ? <section className="live-menu-root"><header><strong>Command Menu</strong><button type="button" aria-label="Close menu" onClick={closeMenu}>×</button></header><div><button type="button" onClick={() => setMenuScreen("SETTINGS")}><strong>Settings</strong></button><button type="button" onClick={() => setMenuScreen("HELP")}><strong>Help</strong></button><button type="button" className="is-danger" onClick={() => { if (globalThis.confirm("End this mission and return to team selection?")) { onNewMission(); closeMenu(); } }}><strong>New Mission</strong></button></div></section> : menuScreen === "SETTINGS" ? <section className="live-menu-subpanel"><header><button type="button" aria-label="Back to menu" onClick={() => setMenuScreen("ROOT")}>‹</button><strong>Settings</strong><button type="button" aria-label="Close menu" onClick={closeMenu}>×</button></header><div className="live-hud-menu-scale"><strong>Desktop scale</strong><div>{(["AUTO", "COMPACT", "STANDARD", "LARGE"] as DesktopBoardScale[]).map((preference) => <button key={preference} type="button" className={desktopBoardScale === preference ? "is-active" : ""} onClick={() => chooseDesktopBoardScale(preference)}>{preference[0] + preference.slice(1).toLowerCase()}</button>)}</div></div></section> : <section className="live-menu-subpanel"><header><button type="button" aria-label="Back to menu" onClick={() => setMenuScreen("ROOT")}>‹</button><strong>Help</strong><button type="button" aria-label="Close menu" onClick={closeMenu}>×</button></header><a href="/space-hulk-death-angel-pwa/rules/death-angel-rulebook.pdf" target="_blank" rel="noreferrer" onClick={closeMenu}><strong>Rules reference</strong></a><button type="button" onClick={() => { onDownloadSave(); closeMenu(); }}><strong>Download diagnostics</strong></button><button type="button" onClick={() => { composeEmail("Space Marine: Death Angel Feedback"); closeMenu(); }}><strong>Send Feedback</strong></button><button type="button" onClick={() => { onDownloadSave(); composeEmail(`Space Hulk: Death Angel Bug Report v${APP_VERSION}`, "Please attach the diagnostics .json file that was just downloaded, then describe what happened."); closeMenu(); }}><strong>Report a Bug</strong></button></section>}</div>}</div></div>
        </div>
