...
const TUTORIAL_HUD_TOUR: Array<{ target: TutorialTarget; title: string; body: string }> = [
  { target: "stats", title: "Support Supply", body: "This shows your unspent Support tokens. Support helps power abilities and rerolls." },
    ...
    <main className={`mission-shell ${choosingActions ? "is-choosing-actions" : ""}`} style={{ "--desktop-ui-scale": desktopUiScale.toFixed(3) } as CSSProperties}>
      ...
      {!travelStage && <section className={`round-command-tray is-${displayedBottomView} ${choosingActions ? "is-choosing" : ""} ${tutorialTarget === "cards" ? "is-tutorial-focus" : ""}`}>
        {choosingActions ? <LiveActionSelection session={session} onChooseOption={onChooseOption} onHoverInspect={onHoverInspect} onDismissHoverInspection={onDismissHoverInspection} tutorialGuide={activeTutorialGuide} /> : <div className="round-command-viewport" onPointerDown={startTraySwipe} onPointerUp={finishTraySwipe} onPointerCancel={() => { traySwipeStart.current = null; onDismissHoverInspection(); }}><div className={`round-command-rail is-${displayedBottomView}`}>
          <div className="round-rail-panel round-rail-cards"><div className="round-rail-content"><LiveActionSelection compact session={session} onChooseOption={onChooseOption} onHoverInspect={onHoverInspect} onDismissHoverInspection={onDismissHoverInspection} tutorialGuide={activeTutorialGuide} /></div><button type="button" className="round-rail-tab artist-info-control" aria-label="Show information panel" onClick={() => setTrayView("status")}><img src="game-art/ui/artist-controls/info.png" alt="" /></button></div>
